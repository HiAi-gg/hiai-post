...
export function getYouTubeAuthUrl(config: YouTubeOAuthConfig, state: string): string {
  const params = new URLSearchParams({
    ...
    state,
    scope: 'https://www.googleapis.com/auth/youtube.upload https://www.googleapis.com/auth/youtube.force-ssl https://www.googleapis.com/auth/youtube.readonly',
  });
...
export async function uploadYouTubeVideo(
  ...
  if (!initResponse.ok) {
    ...
    logger.error({ status: initResponse.status, error: errorText, kind }, 'YouTube resumable init failed');
  ...
  if (!uploadResponse.ok) {
    ...
    logger.error({ status: uploadResponse.status, error: errorText, kind }, 'YouTube upload failed');
  ...
  return {
    ...
    uploadStatus: (uploaded.status?.uploadStatus as YouTubeUploadResult['uploadStatus']) ?? 'processing',
  };
}

export async function fetchVideoFromUrl(url: string): Promise<{ buffer: ArrayBuffer; mimeType: string }> {
...
export async function getYouTubeUploadStatus(
  ...
  return {
    ...
    uploadStatus: (item.status?.uploadStatus as YouTubeUploadResult['uploadStatus']) ?? 'processing',
  };
}
...
export function getYouTubeDefaultPrivacy(kind: YouTubeVideoKind): 'public' | 'unlisted' | 'private' {
