...
function createMockRedis() {
  return {
    ...
    multi: vi.fn(() => ({
      incr: vi.fn(function (this: any) {
        return this;
      }),
      pexpire: vi.fn(function (this: any) {
        return this;
      }),
...
describe("PublishQueue", () => {
  ...
  it("remove calls redis.zrem with correct key", async () => {
    ...
    expect(redis.zrem).toHaveBeenCalledWith("publish_queue:tenant-1", "tenant-1:post-1");
  });
  ...
  it("getScheduled returns sorted items", async () => {
    const redis = createMockRedis();
    redis.zrange.mockResolvedValue(["tenant-1:post-2", "2000", "tenant-1:post-1", "1000"]);
    const queue = new PublishQueue(redis);
