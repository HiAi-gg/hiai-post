...
export function validateContent(
  ...
  if (content.length > rule.maxLength) {
    errors.push(`Content exceeds max length: ${content.length}/${rule.maxLength} chars`);
  }
  ...
  if (emojiCount < rule.emojiRange[0]) {
    errors.push(`Too few emoji: ${emojiCount}/${rule.emojiRange[0]} minimum`);
  }
  if (emojiCount > rule.emojiRange[1]) {
    errors.push(`Too many emoji: ${emojiCount}/${rule.emojiRange[1]} maximum`);
  }

  // Count hashtags
  const hashtagCount = (content.match(/#\w+/g) || []).length;
  if (hashtagCount < rule.hashtagRange[0]) {
    errors.push(`Too few hashtags: ${hashtagCount}/${rule.hashtagRange[0]} minimum`);
  }
  if (hashtagCount > rule.hashtagRange[1]) {
    errors.push(`Too many hashtags: ${hashtagCount}/${rule.hashtagRange[1]} maximum`);
  }
