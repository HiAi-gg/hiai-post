...
async function createMediaContainer(
  ...
  const response = await fetch(`https://graph.facebook.com/v19.0/${accountId}/media?${params}`, {
    method: "POST",
...
async function publishMediaContainer(
  ...
  const response = await fetch(`https://graph.facebook.com/v19.0/${accountId}/media_publish`, {
    method: "POST",
...
export async function publishToInstagram(
  ...
  try {
    const { accessToken, instagramAccountId, content, mediaUrl, mediaType } = options;
    ...
    if (mediaType === "IMAGE" || !mediaType) {
      const containerId = await createMediaContainer(instagramAccountId, accessToken, {
        imageUrl: mediaUrl,
        caption: content,
      });

      await waitForContainer(containerId, accessToken);
      const postId = await publishMediaContainer(instagramAccountId, accessToken, containerId);
    ...
    if (mediaType === "CAROUSEL" && options.carouselItems?.length) {
      ...
      // Wait for all items
      await Promise.all(itemIds.map((id) => waitForContainer(id, accessToken)));
      ...
      const postId = await publishMediaContainer(instagramAccountId, accessToken, carouselData.id);
