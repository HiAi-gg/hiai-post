...
const PLATFORM_LIMITS: Record<string, number> = {
  instagram: 2200, x: 280, linkedin: 3000,
  tiktok: 2200, facebook: 63206, telegram: 4096,
};
  ...
  execute: async ({ inputData, mastra }) => {
    ...
    const variants = await Promise.allSettled(
      inputData.platforms.map(async (platform) => {
        ...
        if (!agent) {
          return { platform, content: inputData.content.slice(0, maxLength), hashtags: inputData.hashtags.slice(0, platform === 'x' ? 2 : 5), maxLength };
        }
    ...
    return { variants: variants.filter((r): r is PromiseFulfilledResult<any> => r.status === 'fulfilled').map(r => r.value) };
  },
});
